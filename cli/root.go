// Package cli builds the infoq command tree on top of the infoq library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: infoq.DefaultConfig()}

	root := &cobra.Command{
		Use:   "infoq",
		Short: "Browse InfoQ software development articles",
		Long: `infoq reads InfoQ through its public RSS feeds.
No API key is required. Records are returned as table, JSON, JSONL, CSV, TSV, or URLs.

infoq is an independent tool and is not affiliated with InfoQ or C4Media.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = infoq.NewClient(a.cfg)
	return nil
}
