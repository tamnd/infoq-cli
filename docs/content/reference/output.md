...
describes what users get. Pick a format with `-o`, or let infoq choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
infoq <command> -o table   # aligned columns for reading
infoq <command> -o jsonl   # one JSON object per line, for piping
infoq <command> -o json    # a single JSON array
infoq <command> -o csv     # spreadsheet friendly
infoq <command> -o tsv     # tab-separated
infoq <command> -o url     # just the URL column
infoq <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
infoq <command> --fields id,title,url
```
...
```bash
infoq <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
infoq <command>            # a table, because this is a terminal
infoq <command> | wc -l    # JSONL, because this is a pipe
```
